...
class ReporteApp:
    ...
    def generar_individuales(self):
        ...
        # Usamos askdirectory en lugar de asksaveasfilename
        output_dir = filedialog.askdirectory(
            title="Seleccione la carpeta donde guardar los reportes individuales"
        )
        if not output_dir:
            return

        # Nombre de la subcarpeta que se creará dentro de la seleccionada
        nombre_carpeta_reportes = "Reportes_Individuales_Estudiantes"
        ruta_final_reportes = os.path.join(output_dir, nombre_carpeta_reportes)

        try:
            # 2. CREAR LA CARPETA SI NO EXISTE
            os.makedirs(ruta_final_reportes, exist_ok=True)
            ...
            # 3. ITERAR Y GENERAR UN PDF POR ESTUDIANTE
            for estudiante in self.data["resultados_estudiantes"]:
                id_estudiante = estudiante.get("ID_Estudiante")
                if not id_estudiante:
                    continue # Saltar si no hay ID_Estudiante

                # Definir la ruta completa del archivo PDF individual
                nombre_archivo = f"{id_estudiante}.pdf"
                filepath = os.path.join(ruta_final_reportes, nombre_archivo)

                # Crear un nuevo canvas para CADA estudiante
                c = canvas.Canvas(filepath, pagesize=letter)
                
                # Generar el contenido del reporte individual
            ...
            messagebox.showinfo("Éxito", 
                                f"Se generaron {archivos_generados} reportes individuales exitosamente en la carpeta:\n{ruta_final_reportes}")

        except Exception as e:
            messagebox.showerror("Error de Generación", f"Ocurrió un error al generar los PDFs: {e}")

    # ... otras funciones de ReporteApp
